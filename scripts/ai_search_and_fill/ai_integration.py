...
parse_bird_info_function = {
    ...
    rarityID, commonName, species, wingspan, size, identifyingMarks, range, description, matingSeason
    'Uncommon', 'Bald Eagle', 'Haliaeetus leucocephalus', 'The average Bald Eagle wingpsan is 6.9 feet', 'The average Bald Eagle is 27.9 inches to 37.8 inches long', 'The Bald Eagle is easily identified by its white capped head and neck, in contrast to the rest of its dark brown coat', 'Found all across North America, except for the most northern regions and below Mexico', 'The Bald Eagle got its name from the Middle English word, \'Balde\', meaning white-headed (not hairless!) These eagles mainly eat fish, and can be found around bodies of water. Though more often than not, they prefer to steal fish from other fishing animals, humans included.', 'Bald Eagle nesting season typically begins in December, and lasts until July. Though their courtship behaviors may begin as early as late Fall, depending on location.
    
    Rarity can only be the options: Common, Legendary, Rare, Uncommon. don't include birdID. description should be info inputted. Any info not included in the text should just be described as N/A. Each piece of information generated by you should have (Information Sourced with AI)"
    """,
    "parameters": {
        "type":"object",
        "properties": {
...
def exa_lookup_and_format(exa_client,bird_info, max_attempts=2):
    ...
    for attempt in range(1, max_attempts+1):
        ...
        results = exa_client.search(
            ...
            rarityID, commonName, species, wingspan, size, identifyingMarks, range, description, photographUrl, matingSeason
        ...
        if not missing_optional:
            ...
            finished_bird = [rarity, common_name, species, wingspan, size,
                              identifying_marks, range_, description, mating_season]
        ...
        finished_bird = [rarity, common_name, species, values["wingspan"], values["size"],
                          values["identifying_marks"], values["range"], description,
